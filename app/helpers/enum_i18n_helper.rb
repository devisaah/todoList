module EnumI18nHelper
  ...
  def array_enum_options_for_select(class_name, enum)
    class_name.send(enum).map do |key, _|
      [I18n.t("activerecord.attributes.#{class_name.model_name.i18n_key}.array_enum.#{enum.to_s}.#{key}"), key]
    end
  end

  def array_enum_i18n(model, enum)
    model.send(enum).map do |key, _|
      I18n.t("activerecord.attributes.#{model.class.model_name.i18n_key}.array_enum.#{enum.to_s}.#{key}")
    end
  end
